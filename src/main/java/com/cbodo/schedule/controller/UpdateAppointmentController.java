...
/**
 * This class handles the logic for the edit appointment form.
 */
public class UpdateAppointmentController implements Initializable {
    ...
    /**
     * Disabled TextField to display selected appointment's ID.
     */
    ...
    /**
     * Appointment object to store the selected appointment.
     */
    private Appointment toUpdate;
    /**
    ...
    /**
     * Handles updating the selected appointment in the database when the save button is pressed.
     * @throws Exception When retrieving User and Contact IDs.
     */
            ...
            if (formIsValid(start, end)) {
                ...
                if (conflicts == null || conflicts.isEmpty()) {
                    ...
                    if (AppointmentDAO.updateAppointment(appointment)) {
                        mainViewController.updateAppointmentData(appointment, toUpdate);
                        alert = "Appointment updated: " + appointment;
                        mainViewController.setAlert("appointment", alert, Color.GREEN);
                    } else {
                        alert = "There was a problem updating the appointment: " + toUpdate.toString();
                        mainViewController.setAlert("appointment", alert, Color.RED);
                    }
                    stage.close();
                } else {
                    errorPane.setVisible(true);
                    formErrorLabel.setText("Appointment conflicts with:\n" + AppointmentDAO.showAppointments(conflicts));
                    formErrorLabel.setTextFill(Color.RED);
                }
            } else {
                showErrors();
    ...
    @FXML public void handleCancel() {
        ...
        if(AlertHelper.alert(title, header, content, "Yes", "No", Alert.AlertType.CONFIRMATION)) {
            String alert = "No changes made to appointment: " + toUpdate.toString();
            mainViewController.setAlert("appointment", alert, Color.BLUE);
            stage.close();
        }
    }

    /**
     * Populates form fields with selected Appointment's values.
     */
    public void populateForm() throws Exception {
        appointmentIdTextField.setText(String.valueOf(toUpdate.getAppointmentId()));
        titleTextField.setText(toUpdate.getTitle());
        descriptionTextField.setText(toUpdate.getDescription());
        typeTextField.setText(toUpdate.getType());
        locationTextField.setText(toUpdate.getLocation());
        startDatePicker.setValue(toUpdate.getStart().toLocalDate());
        endDatePicker.setValue(toUpdate.getEnd().toLocalDate());
        FormHelper.initializeTimeComboBox(toUpdate.getStart(), startHourComboBox, startMinuteComboBox, startPeriodComboBox);
        updateStartTime();
        FormHelper.initializeTimeComboBox(toUpdate.getEnd(), endHourComboBox, endMinuteComboBox, endPeriodComboBox);
        updateEndTime();
        contactComboBox.getSelectionModel().select(ContactDAO.getContactName(toUpdate.getContactId()));
        customerComboBox.getSelectionModel().select(CustomerDAO.getCustomerName(toUpdate.getCustomerId()));
        userComboBox.getSelectionModel().select(UserDAO.getUserName(toUpdate.getUserId()));
    }

    /**
     * Initializes list data and selected appointment passed from main controller,
     * then populates the form with the appointment data.
     * @param controller MainViewController instance.
     * @param appointment Selected appointment.
     */
    public void init(MainViewController controller, Appointment appointment) throws Exception {
        this.toUpdate = appointment;
